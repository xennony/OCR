...
Шаблоны: Использует Jinja2 для рендеринга HTML. Базовый шаблон (base_template) определяет общую структуру страницы, а блоки content заполняются динамическим контентом в зависимости от маршрута.

Фильтры Jinja2: human_readable_time и timestamp_to_human_readable используются для форматирования времени в шаблонах.

Фоновый поток: Запускает функцию record_screenshots_thread из screenshot.py в отдельном потоке для записи скриншотов в фоне.
